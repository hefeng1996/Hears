...
<div style=" background-color: white; border: 1px solid #cccccc;width: 980px; margin:0 auto ;">
    ...
    <img style="float: left;margin: 40px;height: 200px; width: 200px"
            <c:if test="${not empty songlist.img}">
                src="<%= resourcesPath%>images/200songlist/${songlist.img}"
            </c:if>
            <c:if test="${empty songlist.img}">
                src="<%= basePath%>images/default.jpg"
            </c:if>
    />
    <img style=" float: left;margin-top: 40px;" src="<%= basePath%>main/pic/7.jpg"/>
    <p style="margin-top: 40px;"><a style="font-size: x-large;margin-left: 20px">${songlist.name}</a></p>
    <img style="height: 50px;width:50px;" src="<%= resourcesPath%>head/${songlist.user.img}"/>
    <a href="<%= basePath%>user/personalInformation.action?id=${songlist.user.id}" style="margin: 20px">${songlist.user.name}</a>
    <em><fmt:formatDate value="${songlist.createtime}" pattern="yyyy-MM-dd"/> 创建</em>
    <p>
        <a href="javascript:void(0)" id="collectsonglist_img" class="collectsonglist" onclick="collectedSonglist('${songlist.id}')">

            <c:if test="${songlistCollected==0}">
                <img style="margin-top: 10px" id="simg" src="<%= basePath%>main/img/xing.jpg" />
            </c:if>
            <c:if test="${songlistCollected==1}">
                <img style="margin-top: 10px" id="simg" src="<%= basePath%>main/img/xing1.jpg" />
            </c:if>
        </a>
        </p>
    <p style="margin-top: 20px"><em>标签：</em>
        <c:forEach items="${tag}" var="t">
            <em style="margin: 10px"><a href="<%= basePath%>songlist/huayu.action?id=${t.id}">${t.name}</a></em>
        </c:forEach>

    </p>
    <p><em>介绍：</em></p>
    <p>&emsp;&emsp;${songlist.introduce}</p>
    <div style="margin: 40px; margin-top: 50px">
        <p style="font-size: x-large">歌曲列表</p>
        <div style="height: 2px;background-color: red;width: 100%;margin-bottom: 1px"></div>
        <table id="tab" style="width: 900px; border: 1px solid #cccccc">
            <tr style="height: 40px;border: 1px solid #cccccc" bgcolor="#f4f4f4">
                <td style=" width: 20px;padding: 20px"></td>
                <td></td>
                <td width="300px">歌曲标题</td>
                <td>时长</td>
            ...
            <c:forEach items="${song}" var="song" varStatus="s">
                <tr style="border-right: solid 1px #cccccc;border-left: solid 1px #cccccc;height: 35px">
                    ...
                    <td><a href="<%=basePath%>album.action?id=${song.album.id}">${song.album.name}</a></td>
                </tr>
            </c:forEach>
        </table>
    </div>
    ...
    <div style="margin: 60px">
        ...
        <div style="padding: 10px;font-size: 12px" class="m${songlist.id}">
            ...
            <textarea style="display:inline-block;width: 750px;height: 60px;margin-bottom: 20px" id="t${songlist.id}"
                      placeholder="评论"></textarea>
            <button class="btn btn-primary" style="margin-left: 750px" onclick="addcomments(${songlist.id})">评论
            </button>
                ...
                <div id="dc${c.id}" onmouseover="showdelete(${c.id})" onmouseout="hiddendelete(${c.id})">
                    ...
                    <div style="margin-left: 35px;margin-top: 10px;margin-bottom: 10px">
                        ...
                        <c:if test="${sessionScope.user.id==c.user2.id}">
                            <a style="display: none" href="javascript:void(0)" id="sd${c.id}"
                               onclick="deletecomment(${c.id},${songlist.id})">删除</a>
                    ...
                    <div class="c${c.id}" style="display: none">
                        ...
                        <div style="padding: 10px;margin-left: 35px;background-color: #F5F4F3;border: 1px solid #d9d9d9;">
                                    <textarea style="width:750px;height: 60px" id="a${songlist.id}"
                                              placeholder="回复${c.user2.name}："></textarea>

                            <button class="btn btn-primary" style="margin-left: 695px"
                                    onclick="addanswer(${songlist.id},${c.id})">回复
                            </button>
...
//收藏歌单
function collectedSonglist(songlistid){
...
<script>
    ...
    function addcomments(sid) {
        ...
        if(user=="null"||user==""){
            ...
            location.reload();
            $.ajax({
                url: "${pageContext.request.contextPath}/songlist/addsonglistcomment.action",
                type: "post",
                data: "content=" + content + "&songlistid=" + sid,
                success: function () {
    ...
    function addanswer(sid, upperid) {
        ...
        if(user=="null"||user==""){
            ...
            location.reload();
            $.ajax({
                url: "${pageContext.request.contextPath}/songlist/addsonglistanswer.action",
                type: "post",
                data: "content=" + content + "&songlistid=" + sid + "&upperid=" + upperid,
                success: function () {
